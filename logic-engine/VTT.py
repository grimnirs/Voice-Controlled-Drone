...
VALID_FLIGHT_COMMANDS = {
    "arm": [None],
    "fly": ["forward", "backward", "left", "right", "up", "down"],  # Changed to "fly" to match main.py
...
def parse_and_validate(text):
    ...
    # Identify the action
    action = None
    if "arm" in text: action = "arm"
    elif "takeoff" in text: action = "takeoff"
    elif "move" in text or "fly" in text: action = "fly"        # Changed to "fly" to match command_handler
